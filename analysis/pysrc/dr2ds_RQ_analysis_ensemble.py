'''
File: dr2ds_RQ_analysis_ensemble.py
Project: Q_analysis
...
dr2dstau_filename = "dr2ds_RQ_configuration_path_analysis.txt"
...
for i, dir in np.ndenumerate(dirs):
    if isfile(join(dir, output_foldername, run_filename)) & isfile(join(dir, output_foldername, dr2dstau_filename)):
        ...
        if (random_seed_pos_init in [2,3,4,5]) & (b == 0.0):
            dr2ds_temp = pd.read_csv(join(dir, output_foldername, dr2dstau_filename), sep=r"\s+",
                                header=None, skiprows=1, usecols= [0,1,2,3] + [8,9] + list(np.arange(12,22,1))+ list(np.arange(28,37,1)))

            dr2ds_temp.columns = ["dTau2dt_dt_f",	"dTau2dt_dTau2_f",	"dTau2dt_dt",	"dTau2dt_dTau2", "dTau2ds_ds_f", "dTau2ds_dTau2_f",	"ddisplacement2dt_dt_f",	"ddisplacement2dt_ddisplacement2_f",	"ddisplacement2dt_dt",	"ddisplacement2dt_ddisplacement2",	"ddisplacement2ds_ds",	"ddisplacement2ds_ddisplacement2",	"ddisplacement2dsz_ds",	"ddisplacement2dsz_ddisplacement2",	"ddisplacement2ds_ds_f",	"ddisplacement2ds_ddisplacement2_f",	"dt_d_i",	"ddisplacement_i",	"R_i",	"dt_d_abs_i",	"ddisplacement_abs_i",	"R_abs_i",	"ddisplacment2",	"displacement2",	"Tau2"]
...
for b in dr2ds["b"].unique():
    ...
    print(x, y)
    x, y, y_std = np.log10(x), np.log10(y), np.log10(y_std)
    print(np.sum(np.isnan(x_data)), np.sum(np.isnan(y_data)), x, y)
    print(bin_centers_valid)
    bin_centers_valid = bin_centers_valid.astype(bool)
    x = x[bin_centers_valid]
    y = y[bin_centers_valid]
    y_std = y_std[bin_centers_valid]
...
dr2ds_mean = pd.concat([dr2ds_1, dr2ds_2, dr2ds_3, dr2ds_4, dr2ds_5, dr2ds_6, dr2ds_7, dr2ds_8, dr2ds_9, dr2ds_10, dr2ds_11, dr2ds_12], axis=1)

dr2ds_mean.to_csv(join(output_foldername,
                  dr2dstau_filename), index=False, sep='\t', na_rep='nan', mode='a', header=False)